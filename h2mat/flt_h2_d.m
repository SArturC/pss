function output = flt_h2_d(A,B,C1,C2,D21,varargin)
    options = [];
    output  = [];
    if nargin > 1
        if nargin == 6
    ...
    M = rolmipvar(inputs   ,inputs   ,'M','symmetric',vertices,0);
    ...
    h33 =  Z                     ;
    h34 =  Z                     ;
    h35 =  zeros(order,outputs_z);
    
    h44 =  X                     ;
    h45 =  zeros(order,outputs_z);
